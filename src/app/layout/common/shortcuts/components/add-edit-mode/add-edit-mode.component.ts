...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, ReactiveFormsModule, MatFormFieldModule, MatInputModule, MatSlideToggleModule,  MatIconModule, MatButtonModule, NgIf, MatSelectModule, NgForOf, MatOptionModule, NgxMatSelectSearchModule, AsyncPipe]
})
export class AddEditModeComponent {
  ...
  shortcutForm: UntypedFormGroup = this._formBuilder.group({
    ...
    icon: ['Hola mundo', [Validators.required]],
  ...
  public filteredIcons: ReplaySubject<string[]> = new ReplaySubject<string[]>(1);
  ...
  ngOnInit(): void {
    ...
    this._iconsService.icons
      ...
      .subscribe((icons) => {
        ...
        this.icons = iconsList.map( x => `${namespace}:${x}`)
      });
  ...
  protected filterIcons() {
    ...
    this.filteredIcons.next(
      this.icons.filter(string => string.toLowerCase().indexOf(search) > -1)
    );
